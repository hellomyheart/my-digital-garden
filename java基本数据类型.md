...
> 通过以上讲解，可以发现，把变量理解为盒子，也是可行的。变量名可以理解为盒子名，值可以理解为盒子内放的东西。
>
> 定义变量就是给盒子贴一个标签（没有标签的盒子不能使用，已经有的标签不能再贴，也就是不能有两个同标签的盒子），赋值是往盒子里放东西，取值是打开盒子，看一下盒子里面是什么。重新赋值是把盒子原先放到东西拿出来，放入新的东西。

变量概念的引入牵扯到了[[高速公路法则]]。[[高速公路法则下的变量]]。
...
`byte`恰好就是一个字节，而`long`和`double`需要8个字节。![image-20200824194457934](/home/syj/.config/Typora/typora-user-images/image-20200824194457934.png)
...
> 对表示范围感兴趣，想要了解更多的内容，需要百度一下二进制、原码、反码、补码。对于小白，只要记住byte和short的表示范围就行了，为什么是-128~127这种问题，可以暂且放下。需要有一定的基础知识才能完全理解。
...
> 对于变量的作用范围，可以理解为盒子，最外层的括号是一个大盒子，里面的有int型变量i（值为0）和中盒子，（如上）中盒子里面有int型变量x（值为1）和小盒子以及String类型变量s（值为"hi"）,小盒子里面有String类型变量s（值为"hello"）.
>
> 盒子都是有盖子的。可以尝试理解一下。

> 对于小白，以上内容了解即可，没事多看几遍。不要求完全理解和会背，在你往后的学习过程中，你会一点点理解这些内容（牵扯到太多后面的知识），用的多了，你也就会了，不要害怕这些新概念。
...
[高速公路法则下的变量]: 高速公路法则下的变量 "高速公路法则下的变量"
